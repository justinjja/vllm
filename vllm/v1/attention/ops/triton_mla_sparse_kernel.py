...
def _sparse_mla_compute_tile(
    ...
    scale = tl.load(kv_scale) if FP8_KV else 1.0
    NEG_LARGE = -1.0e30
    ...
    for start_indice in range(split_start, split_end, BLOCK_N):
        ...
        if FP8_KV:
            k = (decode_fp8(k) * scale).to(q.dtype)
        ...
        if FP8_KV:
            kpe = (decode_fp8(kpe) * scale).to(q.dtype)
        ...
        if FP8_KV:
            v = (decode_fp8(v) * scale).to(q.dtype)
...
def triton_mla_sparse_attention(
    ...
    Args:
        ...
        kv:        [seq_kv, num_heads_kv=1, dim_qk] bf16 or E4M3 FP8
        ...
        kv_scale: device scalar dequantization scale for FP8 storage.
    ...
    fp8_kv = kv.dtype == torch.float8_e4m3fn
    if fp8_kv:
        assert kv_scale is not None and kv_scale.numel() == 1
        kv = kv.view(torch.uint8)
    else:
        assert kv.dtype in (torch.bfloat16, torch.float16)
