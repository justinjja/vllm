...
"""Relay shared attention state, optionally outside captured model execution."""
...
class PipelineSharing(nn.Module):
    def __init__(
        ...
        if not isinstance(additional_config, dict):
            additional_config = {}
        self.runner_mode = bool(
            additional_config.get("deepseek_v41_pp_runner_sharing", False)
        )
        spec = vllm_config.speculative_config
        if (
            not self.runner_mode
            ...
            or (
                ...
                and (
                    not self.runner_mode
                    or not vllm_config.use_v2_model_runner
                    or not spec.use_dspark()
                    or spec.enable_adaptive_verification
                    or not vllm_config.scheduler_config.async_scheduling
                )
            )
            ...
            raise ValueError(
                "deepseek_v41_pp_sharing requires the V2 runner with "
                "deepseek_v41_pp_runner_sharing, and no microbatching, "
                "context parallelism, KV transfer or external_launcher. "
                "DSpark additionally requires the V2 runner and async scheduling."
            )
        ...
        self.delta_transfers = bool(
            additional_config.get("deepseek_v41_pp_delta_sharing", False)
        )
        self._mirrored_blocks: dict[str, tuple[int, torch.Tensor]] = {}
        self._received_blocks: dict[str, tuple[torch.Tensor, torch.Tensor]] = {}
    ...
    def receive(
        ...
        for route in self.inbound:
            ...
            if route.kind in ("kv", "index_k"):
                ...
                if getattr(self, "delta_transfers", False) and any(
                    r.key == route.key for r in self.outbound
                ):
                    self._received_blocks[route.key] = (
                        tensors[f"{route.key}.ids"],
                        tensors[f"{route.key}.blocks"],
                    )
            else:
    ...
    def send(
        ...
        for route in self.outbound:
            if route.kind in ("kv", "index_k"):
                ...
                if getattr(self, "delta_transfers", False):
                    received = self._received_blocks.pop(route.key, None)
                    block_size = getattr(meta, "cache_block_size", None)
                    if received is not None:
                        # Forward the immutable incoming delta through relay stages.
                        ids, blocks = received
                    elif (
                        block_size is not None
                        and getattr(meta, "max_seq_len", 32768) >= 32768
                    ):
                        cache = layer.kv_cache
                        tracked = self._mirrored_blocks.get(prefix)
                        if tracked is None or tracked[0] != cache.data_ptr():
                            tracked = (
                                cache.data_ptr(),
                                torch.zeros(cache.shape[0], dtype=torch.bool),
                            )
                            self._mirrored_blocks[prefix] = tracked
                        ids, blocks = snapshot_updated_cache_blocks(
                            cache,
                            tables,
                            meta.slot_mapping[:num_tokens],
                            block_size,
                            tracked[1],
                            remaining,
                            write_blocks_cpu=getattr(meta, "write_blocks_cpu", None),
                        )
                    else:
                        ids, blocks = snapshot_cache_blocks(
                            layer.kv_cache, tables, remaining
                        )
                else:
                    ids, blocks = snapshot_cache_blocks(
                        layer.kv_cache, tables, remaining
