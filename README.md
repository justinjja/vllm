...
| --- | --- | --- |
| 2026-09-18 | Located the first recorded nonfinite decoder output in a GLM long-context failure. | With `b1` at 900 MHz, the trace first detected NaNs at `b2` layer 43's output, followed by `b1` layer 44 and the final pipeline stage. The CPU check found no nonfinite values among 808 million stored floating values in layer 43. The failing operation and root cause remain unresolved. [Decoder trace](docs/cmp170hx/glm53.md#first-nonfinite-decoder-output). |
