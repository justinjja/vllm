...
| --- | --- | --- |
| 2026-09-18 | Reproduced `b2` expert-computation failures without peer transfers. | Stock NVFP4 expert kernels produced inconsistent results twice at default clocks, including with `b2` running alone. A 900 MHz repeat failed during eager execution with Xid 79 and a lost PCIe link. The clock cap does not establish reliability. Reset/reload, a host reboot and a BMC power cycle left `b2` absent from PCIe enumeration; eight-GPU serving remains unavailable. The cause remains unresolved. [Isolated expert checks](docs/cmp170hx/glm53.md#isolated-expert-computation). |
| 2026-09-18 | Completed GLM quality and workload checks with both `b1` and `b2` at 900 MHz; tested a clock reversal. | 20/20 objective answers, tools, 8/8 concurrent retrieval, fresh plus four cached 262K retrievals, two sustained long decodes and the full timing matrix passed. C16 steady throughput was 303–304 tokens/s, below target. Restoring only `b2` to default clocks in the same model later stalled the second C16 case. A later isolated test also failed at 900 MHz; the setting is not a reliability fix. [Clock control](docs/cmp170hx/glm53.md#two-card-core-clock-control). |
| 2026-09-18 | Located nonfinite GLM decoder output and traced a subsequent MMU failure. | With `b1` at 900 MHz, NaNs first appeared at `b2` layer 43's output. A broader trace passed fresh and cached 262K retrieval, then failed on the next cached request with `b2` Xid 31 after the layer 56 MoE input check. Stored layer 43 floating data passed integrity checks; the faulting operation and root cause remain unresolved. Recovery restored eight GPUs without a reboot. [Decoder trace](docs/cmp170hx/glm53.md#first-nonfinite-decoder-output). |
| 2026-09-18 | Reproduced independent compute errors on `b1` and tested a lower core-clock limit. | GPU Burn flagged `b1` outside vLLM, after reset/reload and when tested alone. A verified 900 MHz limit passed a 60-second single-card trial and a five-minute eight-GPU trial, with zero mismatches. The model then passed short checks but failed with NaN logits during the first fresh long-context response. The limit did not establish reliable serving. [Compute-integrity evidence](docs/cmp170hx/glm53.md#independent-compute-corruption). |
