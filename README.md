...
| --- | --- | --- |
| 2026-09-18 | Reproduced `b2` expert-computation failures without peer transfers. | Stock NVFP4 expert kernels produced inconsistent results twice at default clocks, including with `b2` running alone. A 900 MHz repeat failed during eager execution with Xid 79 and a lost PCIe link. The clock cap does not establish reliability. Reset/reload and link recovery left seven GPUs visible; serving is stopped pending a host reboot. The cause remains unresolved. [Isolated expert checks](docs/cmp170hx/glm53.md#isolated-expert-computation). |
