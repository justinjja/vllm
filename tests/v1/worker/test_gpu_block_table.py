...
def test_cpu_block_plan_tracks_append_reorder_and_reused_request_slots():
    """PP snapshots must include prefix hits but never the previous slot owner."""
    tables = BlockTables(
        block_sizes=[16, 32],
        max_num_reqs=3,
    ...
    tables.append_block_ids(0, ([2, 4], [3]), overwrite=True)
    tables.append_block_ids(1, ([5], [7]), overwrite=True)
    tables.append_block_ids(0, ([6], [8]), overwrite=False)
    plan = tables.gather_cpu_block_tables([1, 0], 3)
    assert plan is not None
    assert plan[0].tolist() == [[5, -1, -1, -1], [2, 4, 6, -1], [-1] * 4]
    assert plan[1][1].tolist() == [6, 7, 16, 17, -1, -1, -1, -1]
    tables.append_block_ids(0, ([9], []), overwrite=True)
    replaced = tables.gather_cpu_block_tables([0], 1)
    assert replaced[0].tolist() == [[9, -1, -1, -1]]
    assert replaced[1].tolist() == [[-1] * 8]
    # An in-flight PP send owns its snapshot, not a mutable request-state view.
    assert plan[0][1].tolist() == [2, 4, 6, -1]
