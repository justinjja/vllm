...
// Standalone experiment: vector transfers plus a GPU 6 relay for GPU 7.
#include <cuda.h>
...
__device__ bool phase(Shared shared, int rank, unsigned sequence,
  ...
  if constexpr (ParallelFlags) {
    const int peer = wait_ranks == 1 ? 6 : threadIdx.x;
    if (threadIdx.x < wait_ranks) {
    ...
    const int first = wait_ranks == 1 ? 6 : 0;
    const int last = wait_ranks == 1 ? 7 : wait_ranks;
...
__global__ void relay_reduce(const float* input, float* output, Shared shared,
  ...
  for (int i = begin + threadIdx.x * 4; i < end; i += blockDim.x * 4) {
    ...
    value.z += variation;
    value.w += variation;
    store4(shared.input[rank] + i, value);
  }
  ...
  if (rank == 7) {
    ...
    if (!phase<ParallelFlags>(shared, rank, sequence + 2, 1, false, error))
      return;
    for (int i = begin + threadIdx.x * 4; i < end; i += blockDim.x * 4)
      reinterpret_cast<float4*>(output)[i / 4] = load4(shared.relay_output + i);
    if (!phase<ParallelFlags>(shared, rank, sequence + 2, 0, true, error))
      return;
  } else {
    if (!phase<ParallelFlags>(shared, rank, sequence, 8, true, error)) return;
    const int shard = ((width + 27) / 28) * 4;
    ...
    for (int i = begin + rank * shard + threadIdx.x * 4; i < own_end;
      ...
      for (int peer = 0; peer < 8; ++peer) {
        float4 value = load4(shared.input[peer] + i);
        sum.x += value.x;
        sum.y += value.y;
        sum.z += value.z;
        sum.w += value.w;
      }
      store4(shared.partial[rank] + i, sum);
    }
    if (!phase<ParallelFlags>(shared, rank, sequence + 1, 7, true, error))
      return;
    for (int i = begin + threadIdx.x * 4; i < end; i += blockDim.x * 4) {
      const int owner = min((i - begin) / shard, 6);
      const float4 value = load4(shared.partial[owner] + i);
      reinterpret_cast<float4*>(output)[i / 4] = value;
      if (rank == 6) store4(shared.relay_output + i, value);
    }
...
Allocation allocate(int owner, size_t requested) {
  ...
  for (int reader = 0; reader < (owner == 6 ? 8 : 7); ++reader) {
...
int main(int argc, char** argv) {
  ...
  if (devices != ranks || count < 1 || blocks < 1 || blocks > 32 ||
      count % (blocks * 4) != 0)
    return 4;
  ...
  allocations.push_back(allocate(6, count * sizeof(float)));
