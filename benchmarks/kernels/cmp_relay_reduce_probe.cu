...
// Standalone experiment: seven peers plus a GPU 6 relay for GPU 7.
#include <cuda.h>
...
__global__ void relay_reduce(const float* input, float* output, Shared shared,
  ...
  for (int i = begin + threadIdx.x; i < end; i += blockDim.x)
    shared.input[rank][i] = input[i] + variation;
  ...
  if (rank == 7) {
    ...
    for (int i = begin + rank * shard + threadIdx.x; i < own_end;
         i += blockDim.x) {
      float sum = 0.f;
      for (int peer = 0; peer < 8; ++peer) sum += shared.input[peer][i];
      shared.partial[rank][i] = sum;
    }
    if (!phase<ParallelFlags>(shared, rank, sequence + 1, 7, true, error))
      return;
    for (int i = begin + threadIdx.x; i < end; i += blockDim.x) {
      const int owner = min((i - begin) / shard, 6);
      const float value = shared.partial[owner][i];
      output[i] = value;
